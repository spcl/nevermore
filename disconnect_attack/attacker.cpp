...
parse(int argc, char* argv[])
{
  ...
  try
  {
    ...
    options.add_options()
      ...
      ("l,dlid", "destination lid", cxxopts::value<uint32_t>()->default_value("0"), "N")
      ("gid1", "First part of the dest. GID", cxxopts::value<uint64_t>(), "N")
      ("gid2", "Second part of the dest. GID", cxxopts::value<uint64_t>(), "N")
      ("g,sgid", "local port gid index for sending. note that 0-Roce1-IP6, 1-Roce2-IP6, 2-Roce1-IP4, 3-Roce2-IP4", cxxopts::value<uint32_t>()->default_value("1"), "N")

      ("id1", "First cm id", cxxopts::value<uint32_t>(), "N")
      ("id2", "Second  cm id", cxxopts::value<uint32_t>(), "N")
      ("qpn", "target qpn", cxxopts::value<uint32_t>(), "N")
      ("help", "Print help")
     ;
...
//     opens the device (device) and creates a context for further use. 
   ...
   /*   struct ibv_qp_init_attr_ex init_attr_ex = {};
...

    {
      ...
      attr.qkey            = 0x80010000; // can be any constant
...
// move QP to RTS state
...

    {
      ...
      if(allparams.count("sgid")){
        printf("use global routing\n");
        ah_attr.is_global = 1; // must be 1 for roce

        ah_attr.grh.hop_limit = 4; 
        ...
        ah_attr.grh.sgid_index = allparams["sgid"].as<uint32_t>(); //  it sets ip and roce versions
    
      }else{
        printf("We configured sending with IB using only local routing.\n");
        printf("Send to lid %u\n",allparams["dlid"].as<uint32_t>());
      }
...
//    eturns a pointer to the created AH
  ...
  unsigned char payload[mad_size];
  memset(payload,0,sizeof(payload));

  struct mad_header *mad = (struct mad_header*)(void*)(payload);
  struct cm_disconnect_request *disc_request = (struct cm_disconnect_request*)(void*)(mad+1);
  ...
  uint32_t qpn = allparams["qpn"].as<uint32_t>();

  disc_request->lid=htonl(allparams["id1"].as<uint32_t>());
  disc_request->rid=htonl(allparams["id2"].as<uint32_t>());
...
// define the characteristics of the Work Request that we will now submit
  struct ibv_sge sge;
  sge.addr = (uintptr_t)payload; // The address of the buffer to write to (or read from)
  sge.length = sizeof(payload); // The length of the buffer in bytes, i.e. size of the data to send (or to receive)
...
// describes the Work Request to the SEND Queue of the QP (I define an object WR of type ibv_send_wr)
  ...
  wr.wr.ud.remote_qkey = 0x80010000; // sanity check 0x0000000080010000
